...
const DynamicFoodDetailsPage = () => {
  ...
  const fetchFoodDetails = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`Error ${res.status}: Food item not found!`);

      const result = await res.json();
      if (result.success && result.data) {
        setFood(result.data);
        if (result.data.images?.length > 0) setSelectedImage(result.data.images[0]);
      } else {
        setError(result.message || 'Failed to load food details!');
      }
    } catch (err: any) {
  ...
  const fetchReviews = useCallback(async () => {
    if (!foodId) return;
    try {
      const res = await fetch(`${cleanBaseUrl}/api/reviews?foodId=${foodId}`);
      const result = await res.json();
  ...
  const calculateTotalPrice = () => getUnitPrice() * quantity;
  ...
  const handleReviewSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!userName.trim() || !newComment.trim() || !foodId) return;

    try {
      setIsSubmittingReview(true);
      const res = await fetch(`${cleanBaseUrl}/api/reviews`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ foodId, userName, rating: newRating, comment: newComment }),
      });
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-3xl border border-red-100 text-center max-w-md shadow-md">
          ...
          <p className="text-sm text-gray-600 mb-6">{error || 'Food item not found!'}</p>
          <button
            onClick={fetchFoodDetails}
            className="bg-amber-500 hover:bg-amber-600 text-white font-bold px-6 py-2.5 rounded-xl transition text-sm cursor-pointer"
          >
            Try Again
          </button>
        </div>
            ...
            <div className="relative h-80 sm:h-96 w-full rounded-2xl overflow-hidden bg-gray-100 border border-gray-100">
              ...
              {!selectedVariant && hasDiscount && (
                <span className="absolute top-4 right-4 bg-red-500 text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-md">
                  Save ৳{Math.floor(food.price - food.discountPrice!)}
                </span>
              )}
            </div>
          ...
          <div className="flex flex-col justify-between">
            <div>
              ...
              {food.addOns?.length > 0 && (
                <div className="mt-6">
                  <h3 className="text-sm font-bold text-gray-800 mb-2">Add Extras:</h3>
        ...
        <div className="mt-12 bg-white rounded-3xl shadow-sm border border-gray-100 p-6 lg:p-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div className="bg-gray-50 p-6 rounded-2xl border border-gray-200/60 h-fit">
              <h3 className="text-base font-bold text-gray-800 mb-4">Share Your Experience</h3>
              <form onSubmit={handleReviewSubmit} className="space-y-4">
                ...
                <button
                  ...
                  className="w-full bg-gray-900 hover:bg-black text-white font-medium py-2.5 rounded-xl text-sm transition cursor-pointer disabled:opacity-50"
                >
            ...
            <div className="lg:col-span-2 space-y-4">
              {reviews.length === 0 ? (
                ...
                reviews.map((rev, index) => (
                  <div key={rev._id || index} className="p-4 rounded-2xl border border-gray-100 bg-white shadow-xs">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <div className="w-8 h-8 rounded-full bg-amber-100 text-amber-800 font-bold flex items-center justify-center text-xs uppercase">
